            ...
            try:
                ...
                if metadata is not None:
                    logger.opt(ansi=True).info(
                        "<fg #00d9ff>"
                        f"Saved 1 image(s) + embedded metadata to disk for job {message.job_id}: {output_path}"
                        "</>",
                    )
                    ...
                    logger.opt(ansi=True).info(
                        "<fg #00d9ff>"
                        f"Saved 1 image(s) to disk (no metadata) for job {message.job_id}: {output_path}"
                        "</>",
                    )
